...
class DataClass:
    def __init__(self, dataPath, pSeqMaxLen=1024, dSeqMaxLen=128, kmers=-1):
        ...
        train, test = np.array(eSeqData['train']), np.array(eSeqData['test'])
        ...
        for i in range(len(pSeen)):
            if np.any(train[:,0]==i) and np.any(test[:,0]==i): # Test set is used for BindingDB
                pSeen[i] = True
        self.pSeen = np.array(pSeen, dtype=np.bool)
...
class DataClass_normal:
    def __init__(self, dataPath, pSeqMaxLen=1024, dSeqMaxLen=128, kmers=-1, validSize=0.2, sep=' '):
        ...
        self.trainIdList, self.validIdList = train_test_split(
            range(len(eSeqData)), test_size=validSize, random_state=42) # Added random state so that results may be reproducible
        self.testIdList = []
        self.trainSampleNum, self.validSampleNum, self.testSampleNum = len(
        ...
        # Introduced 'pSeen': for each protein boolean indicating whether the protein is in both train/valid set
        train, valid = np.array(eSeqData[:self.trainSampleNum]), np.array(eSeqData[self.trainSampleNum:])
        pSeen = [False] * (len(self.pSeqData))
        for i in range(len(pSeen)):
            if np.any(train[:,0]==i) and np.any(valid[:,0]==i): # Valid is predicted for CElegans/Human
                pSeen[i] = True
        self.pSeen = np.array(pSeen, dtype=np.bool)
