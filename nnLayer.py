...
class TextSPP2(nn.Module):
    ...
    def forward(self, x):
        ...
        # => batchSize × feaSize × size × 1
        x3 = -self.spp1(-x).unsqueeze(dim=3)
        # => batchSize × feaSize × size × 3
        return torch.cat([x1, x2, x3], dim=3)
...
class ResDilaCNNBlock(nn.Module):
    def __init__(self, dilaSize, filterSize=64, dropout=0.15, name='ResDilaCNNBlock'):
        ...
        self.layers = nn.Sequential(
            nn.ELU(),
            nn.Conv1d(filterSize, filterSize, kernel_size=3,
                      padding=dilaSize, dilation=dilaSize),
...
class ResDilaCNNBlocks(nn.Module):
    def __init__(self, feaSize, filterSize, blockNum=10, dilaSizeList=[1, 2, 4, 8, 16], dropout=0.15, name='ResDilaCNNBlocks'):
        ...
        for i in range(blockNum):
            self.blockLayers.add_module(f"ResDilaCNNBlock{i}", ResDilaCNNBlock(
    ...
    def forward(self, x):
        ...
        x = self.blockLayers(x.transpose(1, 2)).transpose(
            1, 2)  # => batchSize × seqLen × filterSize
...
class TextCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bn:
            x = [b(i.transpose(1, 2)).transpose(1, 2)
                 for b, i in zip(self.bns, x)]
        elif self.ln:
            x = [l(i) for l, i in zip(self.lns, x)]
        x = [self.actFunc(i) for i in x]

        if self.reduction == 'pool':
            x = [F.adaptive_max_pool1d(i.transpose(
                1, 2), 1).squeeze(dim=2) for i in x]
...
class TextLSTM(nn.Module):
    def __init__(self, feaSize, hiddenSize, num_layers=1, dropout=0.0, bidirectional=True, name='textBiLSTM'):
        ...
        self.name = name
        self.biLSTM = nn.LSTM(feaSize, hiddenSize, bidirectional=bidirectional,
    ...
    def forward(self, x, xlen=None):
        ...
        if xlen is not None:
            ...
            x = nn.utils.rnn.pack_padded_sequence(
                x[indices], xlen, batch_first=True)
...
class TextGRU(nn.Module):
    def __init__(self, feaSize, hiddenSize, num_layers=1, dropout=0.0, bidirectional=True, name='textBiGRU'):
        ...
        self.name = name
        self.biGRU = nn.GRU(feaSize, hiddenSize, bidirectional=bidirectional,
                            batch_first=True, num_layers=num_layers, dropout=dropout)
    ...
    def forward(self, x):
        for h, bn in zip(self.hiddens, self.bns):
            ...
            if self.bnEveryLayer:
                x = bn(x) if len(x.shape) == 2 else bn(
        ...
        x = self.out(x)
        if self.outBn:
            x = self.bns[-1](x) if len(x.shape) == 2 else self.bns[-1](
                x.transpose(-1, -2)).transpose(-1, -2)
        if self.outAct:
            x = self.actFunc(x)
        if self.outDp:
...
class TextAttention(nn.Module):
    ...
    def forward(self, sequence, reference):
        ...
        # => batchSize × classNum × seqLen
        alpha = self.attn(reference, sequence)
        # => batchSize × classNum × feaSize
        return torch.matmul(alpha, sequence)
...
class LuongAttention(nn.Module):
    ...
    def forward(self, hidden, encoderOutput):
        attentionScore = self.dot_score(
            hidden, encoderOutput).transpose(-1, -2)
        # attentionScore: batchSize × classNum × seq_len
        # => batchSize × classNum × seq_len
...
class SimpleAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        # => batchSize × inSize
        return self.actFunc(torch.matmul(input.transpose(1, 2), alpha).squeeze(2))
...
class InterationAttention(nn.Module):
    ...
    def pooling_attention_s(self, x, y):
        ...
        # => batchSize × seqLen1 × 1
        xAlpha, _ = torch.max(alpha, dim=2, keepdim=True)
        x = torch.matmul(x.transpose(1, 2), F.softmax(xAlpha, dim=1)).squeeze(
            dim=2)  # => batchSize × feaSize1
        # => batchSize × 1 × seqLen2
        yAlpha, _ = torch.max(alpha, dim=1, keepdim=True)
        y = torch.matmul(F.softmax(yAlpha, dim=2), y).squeeze(
            dim=1)  # => batchSize × feaSize2
    ...
    def pooling_attention(self, x, y):
        ...
        # => batchSize × seqLen1 × seqLen2
        alpha = self.W(alpha).squeeze(dim=3)
        # => batchSize × seqLen1 × 1
        xAlpha, _ = torch.max(alpha, dim=2, keepdim=True)
        x = torch.matmul(x.transpose(1, 2), F.softmax(xAlpha, dim=1)).squeeze(
            dim=2)  # => batchSize × feaSize1
    ...
    def concat_simple_attention(self, x, y):
        ...
        u = torch.cat([x, y_pooled.unsqueeze(
            dim=1).expand(-1, x.shape[1], -1)], dim=2)
        # => batchSize × seqLen2 × (feaSize1+feaSize2)
        v = torch.cat([y, x_pooled.unsqueeze(
            dim=1).expand(-1, y.shape[1], -1)], dim=2)
        x, y = self.simpleAttn1(u)[:, :self.feaSize1], self.simpleAttn2(v)[
            :, :self.feaSize2]
        return torch.cat([x, y], dim=1)  # => batchSize × (feaSize1+feaSize2)
    ...
    def plane_attention_s(self, x, y):
        ...
        alpha = F.softmax(alpha.flatten(1, 2), dim=1).unsqueeze(
            dim=1)  # => batchSize × 1 × seqLen1*seqLen2

        x, y = x.unsqueeze(dim=2).expand(-1, -1, y.shape[1], -1), y.unsqueeze(
            dim=1).expand(-1, x.shape[1], -1, -1)  # => batchSize × seqLen1 × seqLen2 × feaSize
        # => batchSize × seqLen1*seqLen2 × (feaSize1+feaSize2)
        xy = torch.cat([x, y], dim=3).flatten(1, 2)
        # => batchSize × (feaSize1+feaSize2)
        return torch.matmul(alpha, xy).squeeze(dim=1)
    ...
    def forward(self, x, y):
        ...
        # => batchSize × (feaSize1+feaSize2)
        return self.dropout(self.attnFunc[self.attnType](x, y))
...
class SelfAttention(nn.Module):
    def __init__(self, featureSize, dk, multiNum, name='selfAttn'):
        ...
        self.WQ = nn.ModuleList([nn.Linear(featureSize, self.dk)
                                 for i in range(multiNum)])
        self.WK = nn.ModuleList([nn.Linear(featureSize, self.dk)
                                 for i in range(multiNum)])
        self.WV = nn.ModuleList([nn.Linear(featureSize, self.dk)
                                 for i in range(multiNum)])
    ...
    def forward(self, x, xlen=None):
        ...
        queries = [self.WQ[i](x) for i in range(self.multiNum)]
        # => multiNum*(batchSize × seqLen × dk)
        keys = [self.WK[i](x) for i in range(self.multiNum)]
        # => multiNum*(batchSize × seqLen × dk)
        values = [self.WV[i](x) for i in range(self.multiNum)]
        scores = [torch.bmm(queries[i], keys[i].transpose(1, 2))/np.sqrt(self.dk)
                  for i in range(self.multiNum)]  # => multiNum*(batchSize × seqLen × seqLen)
        # mask <EOS> padding
        if xlen is not None:
        ...
        z = [torch.bmm(F.softmax(scores[i], dim=2), values[i]) for i in range(
            self.multiNum)]  # => multiNum*(batchSize × seqLen × dk)
...
class SimpleSelfAttention(nn.Module):
    ...
    def forward(self, x, xlen=None):
        ...
        score = torch.bmm(querie, key.transpose(1, 2)) / \
            np.sqrt(self.feaSize)  # => batchSize × seqLen × seqLen
        # mask <EOS> padding
        if xlen is not None:
            # => batchSize × seqLen × seqLen
            mask = torch.zeros(
        ...
        z = torch.bmm(F.softmax(score, dim=2), value)
...
class FFN(nn.Module):
    ...
    def forward(self, x, z):
        # => batchSize × seqLen × feaSize
        z = x + self.dropout(self.layerNorm1(z))
        ffnx = self.Wffn(z)  # => batchSize × seqLen × feaSize
        # => batchSize × seqLen × feaSize
        return z+self.dropout(self.layerNorm2(ffnx))
...
class TextTransformer(nn.Module):
    def __init__(self, layersNum, featureSize, dk, multiNum, seqMaxLen, dropout=0.1, name='textTransformer'):
        ...
        self.posEmb = nn.Parameter(torch.tensor(
            posEmb, dtype=torch.float32), requires_grad=False)  # seqLen × feaSize
        self.transformerLayers = nn.Sequential(
            OrderedDict(
                [('transformer%d' % i, Transformer(featureSize, dk,
                                                   multiNum, seqMaxLen, dropout)) for i in range(layersNum)]
            )
        )
        self.dropout = nn.Dropout(p=dropout)
        self.name = name
    ...
    def forward(self, x, xlen=None):
        ...
        # => batchSize × seqLen × feaSize
        return self.transformerLayers((x, xlen))
...
class Transformer_Wcnn(nn.Module):
    def __init__(self, featureSize, dk, multiNum, dropout=0.1):
        ...
        self.Wcnn = TextCNN(featureSize, featureSize, [
                            1, 3, 5], reduction='None', actFunc=nn.ReLU(), name='Wffn_CNN')
        self.Wffn = nn.Sequential(
    ...
    def forward(self, x):
        ...
        keys = [self.WK[i](x) for i in range(self.multiNum)]
        # => multiNum*(batchSize × seqLen × dk)
        values = [self.WQ[i](x) for i in range(self.multiNum)]
        score = [torch.bmm(queries[i], keys[i].transpose(1, 2))/np.sqrt(self.dk)
                 for i in range(self.multiNum)]  # => multiNum*(batchSize × seqLen × seqLen)
        z = [torch.bmm(F.softmax(score[i], dim=2), values[i]) for i in range(
            self.multiNum)]  # => multiNum*(batchSize × seqLen × dk)
        z = self.WO(torch.cat(z, dim=2))  # => batchSize × seqLen × feaSize
        # => batchSize × seqLen × feaSize
        z = x + self.dropout(self.layerNorm1(z))
        # => batchSize × seqLen × feaSize*3
        ffnx = torch.cat(self.Wcnn(z), dim=2)
...
class TextTransformer_Wcnn(nn.Module):
    def __init__(self, layersNum, featureSize, dk, multiNum, dropout=0.1, name='textTransformer'):
        ...
        self.transformerLayers = nn.Sequential(
            OrderedDict(
                [('transformer%d' % i, Transformer_Wcnn(featureSize,
                                                        dk, multiNum, dropout)) for i in range(layersNum)]
            )
        )
        self.dropout = nn.Dropout(p=dropout)
        self.name = name
...
class HierarchicalSoftmax(nn.Module):
    ...
    def forward(self, x):
        ...
        y += [F.softmax(linear(x), dim=1) for linear in self.linearList[1:]]
        y = torch.cat([y[0][:, i-1].unsqueeze(1)*y[i]
                       for i in range(1, len(y))], dim=1)  # => batchSize × classNum
...
class MultiTaskCEL(nn.Module):
    def __init__(self, lossBalanced=True, ageW=1, genderW=1, name='MTCEL'):
        ...
        self.genderCriterion, self.ageCriterion = nn.CrossEntropyLoss(
        ), nn.CrossEntropyLoss()  # ContinusCrossEntropyLoss()#
        self.genderS, self.ageS = nn.Parameter(torch.zeros(1, dtype=torch.float), requires_grad=lossBalanced), nn.Parameter(
